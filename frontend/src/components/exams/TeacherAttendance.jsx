  ...
  const handleScan = async (text) => {
    ...
    try {
      ...
      if (res.data.message === "Already marked") {
        ...
        // Refresh list if today's date is selected
            ...
            <table className="w-full text-left bg-white">
              <thead className="bg-[#f8f9fa] text-[#004276]">
                <tr>
                  ...
                  <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] hidden sm:table-cell">Communication</th>
                  ...
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-[#f8f9fa] text-slate-400 font-black text-xs flex items-center justify-center shadow-inner group-hover:text-[#004276] transition-colors">
                          {(item.userId?.name || "U").charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="font-bold text-[#31444c] capitalize text-sm">{item.userId?.name || "Unverified Candidate"}</p>
                          <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none mt-1">{item.userId?.class || "No Division"}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5 hidden sm:table-cell">
                      <p className="text-xs font-medium text-slate-400 group-hover:text-slate-600 transition-colors">{item.userId?.email || "n/a"}</p>
                    </td>
                    <td className="px-8 py-5 text-center">
                      <span className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-[#ecfdf5] text-[#059669] text-[9px] font-black tracking-widest uppercase rounded-full shadow-sm">
